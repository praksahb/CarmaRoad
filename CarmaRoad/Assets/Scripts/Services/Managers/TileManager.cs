...
public class TileManager : MonoBehaviour
{
    ...
    public Action<float, AnimalSpawnPosition> SendPositionValue;
    ...
    private void GetPositionX()
    {
        for (int i = 0; i < RoadCollidersList.Count; i++)
        {
            ...
            {
                SendPositionValue?.Invoke(RoadCollidersList[i].bounds.center.x, AnimalSpawnPosition.Left);
            }

            if (RoadCollidersList[i].bounds.center.x > tile2.transform.position.x)
            {
                SendPositionValue?.Invoke(RoadCollidersList[i].bounds.center.x, AnimalSpawnPosition.Right);
            }
